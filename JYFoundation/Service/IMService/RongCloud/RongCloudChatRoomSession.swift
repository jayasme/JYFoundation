//
//  RongCloudChatRoomSession.swift
//  JYFoundation
//
//  Created by Scott Rong on 2018/5/11.
...
fileprivate class RongCloudChatRoomUserListParameter : JYHttpParameter {
    var chatroomId: String
    var count: Int
    var order: Int
    
    required init(chatroomId: String, count: Int, order: Int = 2) {
...
class RongCloudChatRoomUserListModelUser : JYHttpModel {
    var id: String!
    var time: Date!
}

class RongCloudChatRoomUserListModel : JYHttpModel {
    var code: Int!
    var total: Int!
    var users: [RongCloudChatRoomUserListModelUser]!
}

/// The session

public class RongCloudChatRoomSession: JYIMSessionBase {
    ...
    private(set) public var chatRoom: JYIMChatRoom
    ...
    internal init(service: RongCloudService, chatRoom: JYIMChatRoom, fetchHistory: Bool) {
    ...
    public var unreadMessageCount: Int {
        get {
            return Int(client.getUnreadCount(.ConversationType_CHATROOM, targetId: id))
        }
    }
    ...
    public func fetchUserList(maxCount: Int = 500) -> Promise<[JYIMUser]> {
        ...
        let url = "https://api.cn.ronghub.com/chatroom/user/query.json"
        let parameters = RongCloudChatRoomUserListParameter(chatroomId: self.id, count: maxCount)
        let headers: [String: String] = ["App-Key": service.appKey,
                                         "Nonce": nonce,
                                         "Timestamp": timestamp,
                                         "Signature": signature]
        
        return service.httpClient.fetchObject(url, method: .post, header: headers, parameter: parameters, type: RongCloudChatRoomUserListModel.self)
    ...
    public var draft: String? {
        get {
            return client.getTextMessageDraft(.ConversationType_CHATROOM, targetId: id)
        } set (value) {
            if let text = value?.on_blankToNil() {
                client.saveTextMessageDraft(.ConversationType_CHATROOM, targetId: id, content: text)
            } else {
                client.clearTextMessageDraft(.ConversationType_CHATROOM, targetId: id)
            }
        }
    }
    ...
    public func fetchMessages(pageSize: Int = 20) -> [JYIMMessageBase] {
        ...
        if let lastMessageId = messages.first?.messageId {
            ...
            rcMessages = (client.getHistoryMessages(.ConversationType_CHATROOM, targetId: id, oldestMessageId: lastMessageId, count: Int32(pageSize)) as? [RCMessage] ?? []).reversed()
        } else {
            // fetch latest messages
            rcMessages = (client.getLatestMessages(.ConversationType_CHATROOM, targetId: id, count: Int32(pageSize)) as? [RCMessage] ?? []).reversed()
        }
    ...
    public func clearUnreadCount() {
        client.clearMessagesUnreadStatus(.ConversationType_CHATROOM, targetId: id)
            ...
            return Promise<JYIMTextMessageBase> {[weak self] seal in
                ...
                _ = strongSelf.client.sendMessage(
                    ...
                    success: { (messageId) in
                        let rcMessage = RCMessage(type: RCConversationType.ConversationType_CHATROOM,
                                                  targetId: strongSelf.id,
                    ...
                    error: { (errCode, messageId) in
                        let msg = RongCloudTextMessage(messageId: messageId,
                                                        type: RCConversationType.ConversationType_CHATROOM,
                                                        targetId: strongSelf.id,
                                                        content: message)
    ...
    public func sendToTop() {
        client.setConversationToTop(.ConversationType_CHATROOM, targetId: id, isTop: true)
    }
    
    public func leave() -> Promise<Void> {
        return Promise<Void> { seal in
            client.quitChatRoom(id, success: { [weak self] in
                if let strongSelf = self {
                    strongSelf.service.removeSession(session: strongSelf)
                }
                seal.fulfill(())
            }, error: { error in
                let err = ONError.error(code: .RongCloudCode, description: "Leaving chat room error")
                seal.reject(err)
            })
        }
    }
